                ...
                for (log in unsyncedLogs) {
                    try {
                        AppwriteClient.databases.createDocument(
                            ...
                            permissions = listOf(
                                Permission.Companion.read(Role.Companion.user(log.userId)),
                                Permission.Companion.update(Role.Companion.user(log.userId)),
                                Permission.Companion.delete(Role.Companion.user(log.userId))
                            )
                        )

                        repository.markAsSynced(log.id)
                        Log.d(
                            TAG, "AppwriteSync: synced log id=${log.id} " +
                    ...
                    try {
                        val results = AppwriteClient.databases.listDocuments(
                            ...
                            queries = listOf(
                                ...
                                Query.equal("log_type", "sms_received")
                            )
                        )
                        ...
                        Log.e(
                            "CallBridge",
                            "AppwriteSync: read sync error for $phoneNumber — ${e.message}"
                        )
                    }
                }
            } catch (e: Exception) {
